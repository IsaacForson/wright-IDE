...
- When the job IS done and verified: give a short final summary and STOP. Do not invent extra work — no unrequested cleanup, no re-verifying what already passed, no polishing loops. If executing a plan, tick the last step, set its Status line to complete, and END YOUR TURN immediately. Stopping promptly after completion is correct behavior, not laziness.
