...
/** Usage reporter wired by extension.ts; attributes tokens to the real provider. */
let usageReporter: import("@wright/core").UsageReporter | undefined;
export function setUsageReporter(fn: import("@wright/core").UsageReporter): void {
  usageReporter = fn;
}
...
export async function buildFailoverClient(
  ...
  if (opts.forceLocal) {
    ...
    const client = new FailoverModelClient([local], onFailoverStatus, usageReporter);
  ...
  const client = new FailoverModelClient(targets, onFailoverStatus, usageReporter);
